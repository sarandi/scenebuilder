...
import { saveScene, updateScene, getScenes, getScene, deleteScene, reorderScenes, getStory } from "@/lib/api";
...
export default function StoryEditor() {
  ...
  const handleSave = useCallback(async (title: string, content: string, id: number | null = sceneId): Promise<number | null> => {
    ...
    try {
      if (id) {
        await updateScene(token, storyId, id, title, content);
        setSaveStatus("saved");
        setScenes(prev => prev.map(s => s.id === id ? { ...s, title, updatedAt: new Date().toISOString() } : s));
        return id;
      } else {
        const result = await saveScene(token, storyId, title, content);
    ...
    >
      ...
      <div className="editor-column">
        <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 16px", borderBottom: "1px solid var(--border)", background: "var(--surface)", flexShrink: 0 }}>
          ...
          <a
            href="/"
            style={{ color: "var(--fg-muted)", fontSize: "12px", fontFamily: "monospace", textDecoration: "none", flexShrink: 0 }}
          >
            stories
          </a>
          <button
