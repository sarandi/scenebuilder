...
import {
  ...
  type EntityDetail, type EntityType, type EntityTypeField, type EntitySummary, type FieldRefValue,
} from "@/lib/api";
...
export interface EntityEditFormHandle {
  save: () => void;
}
...
interface Props {
  ...
  onSaveStatusChange?: (status: "idle" | "saving" | "saved" | "unsaved") => void;
  onMetaReady?: (meta: EntityMeta) => void;
}

export const EntityEditForm = forwardRef<EntityEditFormHandle, Props>(function EntityEditForm(
  { entityId, autoFocusName, onNameChange, onSaveStatusChange, onMetaReady },
  ref,
) {
  ...
  const setStatus = (s: "idle" | "saving" | "saved" | "unsaved") => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!token) return;
      const [ent, types] = await Promise.all([getEntity(token, entityId), getEntityTypes(token)]);
      const type = types.find((t: EntityType) => t.id === ent.entityTypeId);
      setEntity(ent);
      setNameState(ent.name);
      onNameChange?.(ent.name);
      onMetaReady?.({
        entityTypeId: ent.entityTypeId,
        entityTypeName: ent.entityTypeName ?? undefined,
        entityTypeIcon: ent.entityTypeIcon ?? undefined,
        entityTypeColor: ent.entityTypeColor ?? undefined,
        name: ent.name,
      });
  ...
  const handleSave = useCallback(async (
    ...
    try {
      const fieldValues = fields
        ...
        .map(f => ({
          fieldId: f.id,
          textValue: f.valueType === "text" || f.valueType === "text_array" ? (currentValues[f.id] as string ?? "") : undefined,
  ...
  const triggerAutoSave = useCallback((n: string, v: FieldState, r: FieldRefValue[]) => {
    setStatus("unsaved");
    if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current);
    autoSaveTimer.current = setTimeout(() => handleSave(n, v, r), 2000);
  }, [handleSave]);

  useImperativeHandle(ref, () => ({
    save: () => handleSave(name, values, refValues),
  }), [handleSave, name, values, refValues]);
  ...
  const closePicker = () => { setPickerFieldId(null); setPickerSearch(""); setPickerOptions([]); setPickerCreating(false); };
  ...
  const createAndLink = async (field: EntityTypeField) => {
    ...
    try {
      ...
      const created = await createEntity(token, {
        name: pickerSearch.trim(), entityTypeId: field.refEntityTypeId,
        universeId: entity.universeId, isPublic: true, isSecret: false, fieldValues: [], fieldRefValues: [],
      });
      selectRef(field, { ...created, entityTypeName: undefined, entityTypeIcon: undefined, entityTypeColor: undefined, updatedAt: "" });
    } finally { setPickerCreating(false); }
  };
  ...
  const renderField = (f: EntityTypeField) => {
    ...
    if (f.valueType === "boolean") {
      return (
        <label key={f.id} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
          <input type="checkbox" checked={!!val} onChange={e => setValue(f.id, e.target.checked)} style={{ accentColor: "var(--accent)", width: "14px", height: "14px" }} />
    ...
    if (f.valueType === "entity_ref" || f.valueType === "entity_ref_list") {
      ...
      const filtered = pickerOptions.filter(o => o.name.toLowerCase().includes(pickerSearch.toLowerCase()));
      return (
        <div key={f.id} style={{ position: "relative" }}>
          ...
          {refs.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "6px" }}>
              {refs.map(r => (
                <span key={r.refEntityId} style={{ display: "inline-flex", alignItems: "center", gap: "4px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "3px", padding: "2px 6px", fontSize: "12px", color: "var(--fg)" }}>
                  {r.refEntityName}
                  <button onClick={() => removeRef(f.id, r.refEntityId)} style={{ background: "none", border: "none", color: "var(--fg-muted)", cursor: "pointer", fontSize: "12px", lineHeight: 1, padding: "0 1px" }}>×</button>
                </span>
              ))}
            </div>
          )}
          <button onClick={() => openPicker(f)} style={{ background: "none", border: "1px dashed var(--border)", color: "var(--fg-muted)", cursor: "pointer", fontSize: "11px", fontFamily: "monospace", padding: "3px 8px", borderRadius: "4px", display: "block" }}>
            {f.valueType === "entity_ref" && refs.length ? "change" : "+ link"}
          </button>
          {isOpen && (
            <>
              <div style={{ position: "fixed", inset: 0, zIndex: 105 }} onClick={closePicker} />
              <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, minWidth: "220px", maxWidth: "100%", zIndex: 110, background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden", boxShadow: "0 4px 12px rgba(0,0,0,0.4)" }}>
                <input ref={pickerSearchRef} value={pickerSearch} onChange={e => setPickerSearch(e.target.value)} placeholder="search..." style={{ width: "100%", padding: "6px 10px", background: "var(--surface)", border: "none", borderBottom: "1px solid var(--border)", color: "var(--fg)", fontSize: "12px", fontFamily: "monospace", outline: "none" }} />
                <div style={{ maxHeight: "180px", overflowY: "auto" }}>
                  {filtered.length === 0 && !pickerSearch.trim() && (
                    <div style={{ padding: "8px 10px", color: "var(--fg-muted)", fontSize: "12px", fontStyle: "italic" }}>{pickerOptions.length === 0 ? "loading..." : "no results"}</div>
                  )}
                  {filtered.map(opt => (
                    <div key={opt.id} onClick={() => selectRef(f, opt)} style={{ padding: "6px 10px", fontSize: "13px", cursor: "pointer", color: "var(--fg)", borderBottom: "1px solid var(--border)" }}
                  ...
                  {pickerSearch.trim() && f.refEntityTypeId && (
                    <div onClick={() => createAndLink(f)} style={{ padding: "6px 10px", fontSize: "12px", cursor: pickerCreating ? "default" : "pointer", color: "var(--accent)", borderTop: filtered.length ? "1px solid var(--border)" : "none", fontFamily: "monospace", opacity: pickerCreating ? 0.5 : 1 }}
    ...
    return (
      <div key={f.id}>
        <label style={labelStyle}>{f.label}</label>
        {isLong
          ? <textarea value={(val as string) ?? ""} onChange={e => setValue(f.id, e.target.value)} style={textareaStyle} />
          : <input type={f.valueType === "number" ? "number" : "text"} value={(val as string) ?? ""} onChange={e => setValue(f.id, f.valueType === "number" ? Number(e.target.value) : e.target.value)} style={inputStyle} />
        }
      </div>
    );
  };
  ...
  if (!entity) {
    return <p style={{ color: "var(--fg-muted)", fontFamily: "monospace", fontSize: "13px" }}>loading...</p>;
  }
